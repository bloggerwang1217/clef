...
def convert_dotted_triplets(sequence: str) -> str:
    ...
    for dotted_triplet, binary in DOTTED_TRIPLET_TO_BINARY.items():
        ...
        pattern = rf'({re.escape(dotted_triplet)})([A-Ga-gr\[\]_])'
...
def expand_tuplets_to_zeng_vocab(sequence: str) -> str:
    """
    Expand/merge tuplet notes to match Zeng's vocabulary.

    This is a WORKAROUND for clef-piano-base preset only.
    clef-piano-full should extend the vocabulary with native tuplet support.

    Two-pass strategy:
    1. Expand dotted tuplets: n. → [n + n*2] (tied)
    2. Convert tuplets to binary/triplet:
       - Expansion (7,5,11,15-tuplets): n notes → (n+1) notes, last one tied
       - Merge (9,13,17,19-tuplets): n notes → (n-1) notes, one note dropped

    Args:
        sequence: Kern sequence with tuplet tokens

    Returns:
        Transformed sequence using only Zeng vocabulary durations
    """
    # First pass: convert dotted triplets to binary (exact, no tie)
    sequence = convert_dotted_triplets(sequence)
    # Second pass: expand dotted tuplets to tied pairs
    sequence = expand_dotted_tuplets(sequence)
    lines = sequence.split('\n')
    result_lines = []

    # Track tuplet counts per spine
    expand_counts: Dict[int, Dict[str, int]] = {}  # spine_idx -> {duration -> count}
    merge_counts: Dict[int, Dict[str, int]] = {}

    for line in lines:
        # Skip control lines
        if line.startswith(('=', '*', '!')) or line.strip() == '':
            result_lines.append(line)
            # Reset counts at barlines
            if line.startswith('='):
                expand_counts = {}
                merge_counts = {}
            continue
        ...
        extra_line_tokens = []
        need_extra_line = False
        skip_tokens = []  # For merge strategy

        for i, token in enumerate(tokens):
            # Initialize spine tracking
            if i not in expand_counts:
                expand_counts[i] = {}
            if i not in merge_counts:
                merge_counts[i] = {}

            # Extract leading markers if present:
            # - tie marker: [
            # - slur markers: ( )
            # - phrase markers: { }
            # These need to be preserved but moved out of the way for duration matching
            prefix_markers = ''
            working_token = token
            ...
            # Also handle suffix markers that appear before the pitch
            # e.g., (56ELL has ( prefix and no suffix before duration

            # Check for expansion tuplets (7, 5, 11, 15)
            expand_match = None
            for dur, (target, group_size) in EXPAND_MAPS.items():
                if re.match(rf'^{dur}[A-Ga-grnqQP\[\]_]', working_token):
                    expand_match = (dur, target, group_size)
                    break

            if expand_match:
                dur, target, group_size = expand_match
                converted = re.sub(rf'^{dur}', target, working_token)
                # Restore prefix markers
                converted = prefix_markers + converted

                # Track count
                if dur not in expand_counts[i]:
                    expand_counts[i][dur] = 0
                expand_counts[i][dur] += 1

                # Every group_size-th note, add extra tied note
                if expand_counts[i][dur] == group_size:
                    need_extra_line = True
                    # Add tie start to current token
                    if '[' not in converted:
                        converted = '[' + converted
                    converted_tokens.append(converted)
                    # Create tie end for extra line
                    extra_converted = re.sub(rf'^{dur}', target, working_token)
            ...
            # Check for merge tuplets (9, 13, 17, 19)
            merge_match = None
            for dur, (target, group_size) in MERGE_MAPS.items():
                if re.match(rf'^{dur}[A-Ga-grnqQP\[\]_]', working_token):
                    merge_match = (dur, target, group_size)
                    break

            if merge_match:
                dur, target, group_size = merge_match
                converted = re.sub(rf'^{dur}', target, working_token)
                # Restore prefix markers
                converted = prefix_markers + converted

                # Track count
                if dur not in merge_counts[i]:
                    merge_counts[i][dur] = 0
                merge_counts[i][dur] += 1

                # Skip every group_size-th note (merge into previous)
                if merge_counts[i][dur] == group_size:
                    # Extend previous note duration (simplified: just skip this note)
                    skip_tokens.append(i)
                    merge_counts[i][dur] = 0
                else:
                    converted_tokens.append(converted)
                    extra_line_tokens.append('.')
            ...
            converted_tokens.append(token)
            extra_line_tokens.append('.')
            # Reset tuplet counts for this spine
            expand_counts[i] = {}
            merge_counts[i] = {}

        # Handle skipped tokens (for merge strategy)
        if skip_tokens:
            # For now, just use '.' placeholder for skipped tokens
            final_tokens = []
            for i, t in enumerate(converted_tokens):
                if i in skip_tokens:
                    continue  # Skip this token entirely
                final_tokens.append(t)
            if final_tokens:
                result_lines.append('\t'.join(final_tokens) if len(final_tokens) > 1 else final_tokens[0])
        else:
            result_lines.append('\t'.join(converted_tokens) if len(converted_tokens) > 1 else converted_tokens[0])
...
# Extreme pitches outside this range → <unk>

EXTREME_LOW_PITCHES = {'AAAA', 'BBBB', 'CCCC', 'DDDD', 'EEEE', 'FFFF', 'GGGG'}
EXTREME_HIGH_PITCHES = {'aaaaa', 'bbbbb', 'ccccc', 'ddddd', 'eeeee', 'fffff', 'ggggg'}


def mark_extreme_pitches(token: str) -> str:
    """
    Mark extreme pitches (outside piano range) as <unk>.

    Piano range: A0 (AAA) to C8 (cccc)
    Extreme low: AAAA and below (sub-sub-contra)
    Extreme high: ccccc and above (six-line)

    These are likely encoding errors from MuseSyn user uploads.

    Args:
        token: Kern token

    Returns:
        Original token or '<unk>' if extreme pitch detected
    """
    ...
    if match:
        ...
        # Check for 5+ repeated letters (definitely out of range)
        if len(pitch_letters) >= 5:
            return '<unk>'

        # Check for 4-letter extreme pitches
        if len(pitch_letters) == 4:
            # Normalize to check against known extremes
            base = pitch_letters[0]
            if all(c == base for c in pitch_letters):
                if pitch_letters.isupper() and pitch_letters in EXTREME_LOW_PITCHES:
                    return '<unk>'
                # Note: 4-letter lowercase like 'cccc' is valid (C8 on piano)
                # Only 'aaaaa' etc (5+ letters) are truly extreme
            ...
            if ' ' in token:
                ...
                for note in notes:
                    if note == '.' or not note:
                        ...
                        note = mark_extreme_pitches(note)
                        processed_notes.append(note)
                ...
                token = mark_extreme_pitches(token)
                processed_tokens.append(token)
