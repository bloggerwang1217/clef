...
from src.audio.mel import load_mel_config, duration_to_frames
...
def create_manifest(
    ...
    for key, entry in tqdm(metadata.items(), desc="Building manifest"):
        ...
        # Calculate n_frames from duration (faster than loading mel)
        n_frames = duration_to_frames(entry["duration_sec"], sample_rate, hop_length)
