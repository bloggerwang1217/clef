...
| Study 2 | Breadth (廣度) | URMP + Slakh2100 (Ensemble) | MT3 + MuseScore 4 | Ensemble MV2H > 60% |

### 核心論點

> 「只要有譜，我就能生成訓練資料；只要有訓練資料，我的 VLM 就能學會分離並轉錄任何樂器組合。」
...
| **Auxiliary Loss** | 不使用 | ✅ Instrument Auxiliary Loss |
...
| 2026 Jun-Aug | 衝刺 Multi-instrument 實驗 | 跨樂器 TDR + Aux Loss |
| 2026 Aug-Sep | 撰寫 ICLR 論文 | ISMIR 放榜（通常 8 月底）|
| **2026 Sep-Oct** | **投稿 ICLR 2027** | Multi-instrument A2S 論文 |
...
**Title Idea**：
> _Domain-Invariant Music Representation Learning via Video-VLM Architecture and Timbre Randomization_

**關鍵賣點（ICLR 評審喜歡的）**：
1. **Sim-to-Real Transfer**：用合成數據訓練，在真實錄音上表現良好
2. **Representation Disentanglement**：Aux Loss 強迫編碼器分離「音色」與「音高」
3. **Zero-shot Generalization**：對未見過的樂器組合仍能正確分離轉錄
